...
public class WheelView extends View implements IWheelViewSetting {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        maxScrollOffset = (items.length - 1) * itemHeight;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (distanceY < 0) { // 向上滑动
                    if (totalMoveY + distanceY < -maxScrollOffset) {
                        totalMoveY = -maxScrollOffset;
                        break;
                    }
                } else if (distanceY > 0) { // 向下滑动
                    if (totalMoveY + distanceY > 0) {
                        totalMoveY = 0;
                        break;
                    }
                } else {
                ...
                direction = distanceY / Math.abs(distanceY);
                ...
                if (rectF.contains(currentX, currentY)) {
                    ...
                    lastX = currentX;
                    lastY = currentY;
                    updateByTotalMoveY(totalMoveY + distanceY, direction);
                }
    ...
    private void updateByTotalMoveY(int totalMoveY, int direction) {
        if (totalMoveY < -maxScrollOffset) { // 向上滑动
            totalMoveY = -maxScrollOffset;
        } else if (totalMoveY > 0) { // 向下滑动
            totalMoveY = 0;
        }
    ...
    public void computeScroll() {
        if (mOverScroller.computeScrollOffset()) { // true说明滚动尚未完成，false说明滚动已经完成
